...
@Monthly_Fee_Router.put("/Update_Greater_than_month_Monthly_Fee")
async def Update_Monthly_Fee(month:int, scheme: Update_Monthly_Fee_Scheme, session:Session = Depends(init_session)):
    monthly_fee = session.query(Monthly_Fee).filter(extract("month",Monthly_Fee.Date) >= month).all()
